...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="Relation">
    namespace <xsl:value-of select="Entity/Schema" />.<xsl:value-of select="Entity/LogicalPackage" />.<xsl:value-of select="Entity/LogicalModule" />.Collections
    {
      public class <xsl:value-of select="Entity/LogicalName" />Collection : System.Collections.Generic.List&lt;<xsl:value-of select="Entity/LogicalName" />&gt;
      {

        ///
        /// Default Constructor
        ///
        public <xsl:value-of select="Entity/LogicalName" />Collection()
        {
        }
